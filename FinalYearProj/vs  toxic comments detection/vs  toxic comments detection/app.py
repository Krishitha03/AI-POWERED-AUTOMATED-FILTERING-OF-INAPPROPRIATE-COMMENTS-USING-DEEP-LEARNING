...
def predict1():
    ...
    if request.method == "POST":
        ...
        # Convert stored predictions to DataFrame
        output_df = pd.DataFrame(predictions_list)

        return render_template(
            "index.html",
            tables=[output_df.to_html(classes="data", index=False)],  # Removed index
...
def get_toxic_comments():
    ...
    ] = binary_predictions[toxic_indices].tolist()
    ...
    # Store the DataFrame globally for download
    global toxic_comments_df_global
    toxic_comments_df_global = toxic_comments_df
